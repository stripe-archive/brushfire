...
class BrierScoreTest extends WordSpec with Matchers {
  ...
  "A BrierDecompositionError containing one example" should {
    // A single true example that's predicted to be true with probability 0.8 and false with probability 0.2.
    val predicted = List(Map[String, Long]("true" -> 1L, "false" -> 0L), Map[String, Long]("true" -> 1L, "false" -> 0L), Map[String, Long]("true" -> 1L, "false" -> 0L), Map[String, Long]("true" -> 1L, "false" -> 0L), Map[String, Long]("true" -> 0L, "false" -> 1L))
    val actual = Map[String, Long]("true" -> 1L, "false" -> 0L)
    val totalCount = 1L

    val bde = BrierDecompositionError[String]
    val bs = bde.create(actual, predicted)
