...
case class Tree[K, V, T, A: Semigroup](root: Node[K, V, T, A]) {
   ...
   */
  def annotate[A1: Semigroup](f: T => A1): Tree[K, V, T, A1] =
    mapLeaves { (t, _) => (t, f(t)) }
   ...
   */
  def mapAnnotation[A1: Semigroup](f: A => A1): Tree[K, V, T, A1] =
    mapLeaves { (t, a) => (t, f(a)) }

  /**
   * Maps the feature keys used to split the `Tree` using `f`.
   */
  def mapKeys[K1](f: K => K1): Tree[K1, V, T, A] =
    mapSplits { (k, p) => (f(k), p) }
   ...
   */
  def mapPredicates[V1: Ordering](f: V => V1): Tree[K, V1, T, A] =
   ...
   */
  def prune[P, E](validationData: Map[Int, T], voter: Voter[T, P], error: Error[T, P, E])(implicit targetMonoid: Monoid[T], errorOrdering: Ordering[E]): Tree[K, V, T, A] = {
   ...
   */
  def growByLeafIndex(fn: Int => Seq[(K, Predicate[V], T, A)]): Tree[K, V, T, A] = {
    var newIndex = -1
    def incrIndex(): Int = {
      newIndex += 1
      newIndex
   ...
   * left as-is.
   */
  def updateByLeafIndex(fn: Int => Option[Node[K, V, T, A]]): Tree[K, V, T, A] = {
